...
namespace ROD_engine_DX11
{
    ...
    public class ThirdPersonCamera
    {
        ...
        public void Perspective(float fovx, float aspect, float znear, float zfar)
        {
            ...
            float e = 1.0f / (float)Math.Tan(ToRadians(fovx) / 2.0f);
        ...
        private void UpdateOrientation(float elapsedTimeSec)
        {
            ...
            float heading = ToRadians(headingDegrees);
            float pitch = ToRadians(pitchDegrees);
            Quaternion rotation = Quaternion.Identity;

            if (heading != 0.0f)
            {
                rotation=Quaternion.RotationAxis(targetYAxis, heading);
                orientation=Quaternion.Add(rotation, orientation);
            }
            ...
            if (pitch != 0.0f)
            {
                ...
                orientation=Quaternion.Add(orientation, rotation);
            }
        }

        private void UpdateViewMatrix()
        {
            viewMatrix=Matrix.RotationQuaternion(orientation);
